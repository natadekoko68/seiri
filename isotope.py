...
df.loc[df["label"].str.contains("standard"), "b_t"] = (df.loc[df["label"].str.contains("standard"), "Mean"] - blank) / (
        count_sum - blank) * 100
df.loc[df["label"].str.contains("sample"), "b_t"] = (df.loc[df["label"].str.contains("sample"), "Mean"] - blank) / (
        count_sum - blank) * 100
...
plt.text(13, 20,
         "y = " + str(round(slope, 3)) + " log(x) " + " + " + str(round(intercept, 3)) + " \n ( R$^{2}$ = " + str(
             round(r_value ** 2, 3)) + " )")
