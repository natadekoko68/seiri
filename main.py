...
no_tnf_4_raw = np.array(
    [5042.158, 4131.535, 4279.886, 5103.673, 5615.572, 5254.339, 4895.331, 4430.852, 5521.278, 5624.594])
no_tnf_5_raw = np.array(
    [4455.874, 5297.917, 4181.959, 4194.904, 4793.028, 4707.881, 5653.838, 5813.804, 5170.854, 5284.536, 4749.668])
...
"""0.5"""
tnf_half_1_raw = np.array(
    [6622.535, 6178.528, 6950.033, 5126.108, 6150.252, 7518.797, 6646.664, 8539.671, 7757.857, 3273.386])
...
tnf_half_4_raw = np.array(
    [5337.65, 5359.75, 7318.474, 6542.086, 6127.423, 5477.664, 7039.975, 5908.263, 6441.952, 5718.95, 7660.726])
...
tnf_10_2_raw = np.array(
    [6106.227, 9569.013, 9374.088, 14920.978, 6373.751, 9627.45, 6034.653, 5534.27, 6834.111, 5489.461])
...
temps = []
for i, v in df.groupby("category"):
    temp_temp = []
    temp = df.groupby("category").get_group(i).reset_index(drop=True)
    for j in range(len(temp)):
        temp_temp.append(temp.loc[j, "value"])
    temps.append(temp_temp)
res = st.tukey_hsd(temps[0], temps[1], temps[2])
print(res)
